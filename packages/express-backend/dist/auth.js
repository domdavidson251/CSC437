...
function registerUser(req, res) {
  ...
  if (!username || !pwd) {
    ...
    import_credentials.default.create(username, pwd).then((creds) => generateAccessToken(creds.username)).then((token) => {
      ...
      import_profiles.default.create(profile);
    });
  }
}
function loginUser(req, res) {
  ...
  if (!username || !pwd) {
    ...
    import_credentials.default.verify(username, pwd).then((goodUser) => generateAccessToken(goodUser.toString())).then((token) => res.status(200).send({ token })).catch((error) => res.status(401).send("Unauthorized"));
  }
}
